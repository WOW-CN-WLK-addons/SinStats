    ...
    if event == "PLAYER_ENTERING_WORLD" then
        ...
        for skillIndex = 1, GetNumSkillLines() do
            ...
            if ((skillName ~= nil) and (not isHeader) and (skillMaxRank > 1) and (string.find(skillName, _G["LANGUAGE"]) ~= 1)) then
                tinsert(Ns.DefaultOrder, { stat=skillName, events={ CHAT_MSG_SKILL = true }, spellclass="Skill", widget={ type="CheckBox" }, options={ Show=true }, subgroup=true, activeonload=true })
